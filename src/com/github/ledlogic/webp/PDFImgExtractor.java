...
public class PDFImgExtractor extends PDFGraphicsStreamEngine {
    ...
    private PDDocument document;  // Store document for page rendering
    ...
    public void extractImages(File pdfFile) {
        try (PDDocument document = PDDocument.load(pdfFile)) {
            this.document = document;  // Store for page rendering
        ...
        for (Map.Entry<String, ImageFingerprint> entry : imageFingerprints.entrySet()) {
            ...
            if (fp.occurrenceCount >= 3) {
                ...
                System.out.println("  Furniture detected: " + fp.width + "x" + fp.height + 
                                 "px (appeared " + fp.occurrenceCount + " times)");
            }
        }
    ...
    private boolean shouldRenderPage(int pageNum) {
        ...
        if (DEBUG_MODE && pageNum >= 115 && pageNum <= 120) {
            System.out.println("    [CONFIG] Page " + pageNum + " NOT in PAGES_TO_RENDER array: " + java.util.Arrays.toString(PAGES_TO_RENDER));
        }
        return false;
    }
    ...
    private void renderPageAsImage(PDPage page) {
        try {
            ...
            imageCounter++;
            String fileName = String.format("%s_page%d_rendered_%dx%d.png", 
                                           baseName, currentPageNum, width, height);
    ...
    private void processImage(PDImageXObject image, String imageName) {
        try {
            ...
            if (width < MIN_DIMENSION || height < MIN_DIMENSION) {
                if (DEBUG_MODE) {
                    System.out.println("  [SKIP: TOO SMALL] " + imageName + " (" + width + "x" + height + "px) - min: " + MIN_DIMENSION + "px");
                }
                return;
            }
            ...
            if (currentPageChecksums.contains(checksumKey)) {
                if (DEBUG_MODE) {
                    System.out.println("  [SKIP: DUPLICATE ON PAGE] " + imageName + " (" + width + "x" + height + "px)");
                }
                return;
            }
            ...
            if (imageFingerprints.containsKey(fingerprintKey)) {
                ...
                System.out.println("  Duplicate detected: " + imageName + " (" + width + "x" + height + "px) - occurrence #" + 
                                 imageFingerprints.get(fingerprintKey).occurrenceCount);
     ...
     * Samples pixels to check if there's significant color saturation.
     */
    ...
    /**
     * Calculate a simple checksum for an image to detect identical content.
